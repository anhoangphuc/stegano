...
def get_gray_image():
    filename = f'gray.{session["image_type"]}'
    return os.path.join('./app', 'static', 'app_image', filename)

def read_content(filename):
    filename = os.path.join('app', 'templates', f'{filename}.html')
...
def visual_message(message, image_name):
    message_length = len(message)
    width = int(pow(message_length, 0.5))
    if width * width != message_length:
        width += 1
    height = width
    ...
    mode = 'L'
    color = 0
    blank_image = Image.new(mode, size, color)

    #change value of picture
    i = j = 0
    for message_bit in message:
        if message_bit == 1:
            blank_image.putpixel((i, j), 255)
        j = (j + 1) % width
        if (j == 0):
            i = (i + 1) % height
    
    blank_image.save(image_name)
